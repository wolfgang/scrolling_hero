...
pub fn dungeon_with_one_path2(width: usize, height: usize, rng: &mut StdRng) -> DungeonLayout {
    let mut dungeon = generate_dungeon_init(width, height);
    generate_dungeon_path2(&mut dungeon, rng);
    dungeon
}
...
fn generate_dungeon_path2(dungeon: &mut DungeonLayout, rng: &mut StdRng) {
    let width = dungeon[0].len();
    let height = dungeon.len();
