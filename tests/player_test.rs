...
it "player2_has_initial_position" {
    let player = Player::new(10, 20, &move_predicate);
    assert_eq!((10, 20), player.position());
}


it "player2_move_left_until_zero_x" {
    let player = Player::new(2, 0, &move_predicate);
    player.move_left();
...
it "move_right_until_max_x" {
    let player = Player::new(MAX_X - 2, 0, &move_predicate);
    player.move_right();
...
it "move_down_until_max_y" {
    let player = Player::new(0, MAX_Y - 2, &move_predicate);
    player.move_down();
...
it "can_move_while_borrowed_immutably" {
    let player = Player::new(0, 0, &move_predicate);
